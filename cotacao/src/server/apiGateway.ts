const BASE_URL = {
    countryFlags: 'http://webservices.oorsprong.org/websamples.countryinfo/CountryInfoService.wso?WSDL',
    cotacao: ''
}
...
async function getCountryFlag(countryCode: string): Promise<string> {
    try {
        ...
        const result = await client.GetCountryInfo({ sCountryISOCode: countryCode });
